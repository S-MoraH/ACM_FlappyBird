...
#Note: using SVG Coordinate System
...
class Bird:
    ...
    # constructor with a birds intial values
    def __init__(self, x, y):
        self.x = x              # x-coordinate of the bird
        self.y = y              # y-coordinate of the bird
        self.tilt = 0           # how much the bird image is titled, starts with 0 tilt
        self.tick_count = 0     # tick_count, vel (velocity), and height will be used for the physics of how the bird moves
    ...
    #Note: The bird is not moving horizontally, only vertically. The pipe along with the land will move horizontally to make it look like our bird is moving horizontally.
    ...
    # create a perfect collision box for the FlappyBird (bitmask)
    def get_mask(self):
        
        # masking creates a 2d boolean array of our FlappyBird image, which stores whether a pixel is present in a certain part of the FlappyBird image.
        # Using masks helps make it so that we can calculate exactly if the Bird and pipe actually have overlapping pixels.
...
#Note: win.blit is a method of our win (window) object and blit is a method to draw an image onto the pygame window
...
class Pipe: 
    ...
    #speed of the pipe approaching the bird
    def move(self): 
        self.x -= self.VEL
    ...
    def collide(self, bird, win):
        ...
        #create mask for pipes
        top_mask = pygame.mask.from_surface(self.PIPE_TOP)
        bottom_mask = pygame.mask.from_surface(self.PIPE_BOTTOM)

        # (tuples that) calculate how far away the corners are away from each other
...
def draw_window(win, birds, pipes, base, score, exit_button=None, gen=None, threshold_msg=None): 
    ...
    #if there is an exit button
    if exit_button is not None:
        exit_button.draw(win) # draw exit button 

    #if there is a threshold msg
...
def main(genomes, config):
    global gen # declares a global variable, ensures that it isn't recreated each time main is called
    ...
    #bird = Bird(230, 350)           #bird starting position
